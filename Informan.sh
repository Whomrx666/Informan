...
whois() {
	echo "\aInput Domain [ex : example.com]"
                echo $white "╭─"$green"MrX@localhost"$cyan" ~/Informan "$white
                read -p " ╰─>~#  "  target
...
revdnslook() {
	echo "\aInput Domain [ex : example.com]"
		echo $white "╭─"$green"Mr.X@localhost"$cyan" ~/Informan "$white
		read -p " ╰─>~#  "  target
...
echo $white"     MNO!!"$red"         [Mr.X]"$red"          MNNOO!    "
...
if [ $select -eq 00 ];
	...
	Author by       : Mr.X
        version         : 1.0
        made on a date  : 26 - Sep - 2023
        Blog            : https://whomrxhackers.blogspot.com
        Github          : https://github.com/Whomrx666
        Youtube         : whomrx666 
        Whatsapp        : +6287855190571

                        + THANKS TO MY TEAM +

            + BIRD INTELLEGENT NATION CYBER +
